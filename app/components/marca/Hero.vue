<template>
    <DefaultSection class="relative">
        <picture class="w-full">
            <source media="(min-width: 1440px)"
                srcset="/images/marcas/Tzadik-Michigan-Hanomag-Chery-Desktop-XL.webp">
            <source media="(min-width: 1080px)"
                srcset="/images/marcas/Tzadik-Michigan-Hanomag-Chery-Desktop.webp">
            <source media="(min-width: 768px)"
                srcset="/images/marcas/Tzadik-Michigan-Hanomag-Chery-Tablet.webp">
            <img src="/images/marcas/Tzadik-Michigan-Hanomag-Chery-Mobile.webp"
                alt="Tzadik Servicios Integrales Maquinaria"
                class="w-full h-[28rem] md:h-80 lg:h-[25.5rem] xxl:h-[33.25rem] object-cover object-left">
        </picture>
        <div
            class="w-full h-full flex flex-col items-center justify-center lg:justify-start gap-3 lg:gap-4 absolute text-light p-5 lg:pt-20 xxl:pt-[7.5rem]">
            <HeadingH1 class="max-w-80 md:max-w-full">
                NUESTRAS MARCAS LÍDERES EN MAQUINARIA
            </HeadingH1>
            <p
                class="max-w-96 md:max-w-[33rem] lg:max-w-[45rem] xxl:max-w-[48rem] text-center text-sm md:text-base lg:text-xl xxl:text-2xl font-medium">
                Descubre las mejores marcas de maquinaria vial y agrícola que ofrecemos para impulsar tu productividad y eficiencia.
            </p>
        </div>
    </DefaultSection>
</template>