<template>
    <NuxtLink to="#" class="h-full min-h-[23rem] flex flex-col relative primary-shadow rounded-xl">
        <NuxtImg :src="imageUrl" :alt="product.titulo" class="w-full h-48 object-cover rounded-t-xl" />
        ...
        <div class="h-full min-h-[11rem] flex flex-col justify-between gap-3 p-4">
            <div class="flex flex-col gap-3">
                <p class="font-teko font-semibold text-[1.125rem]">{{ product.titulo }}</p>
                <p class="text-xs">{{ product.descripcion_corta }}</p>
            </div>
